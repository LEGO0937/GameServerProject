...
void Status_Draw( HDC hdc ) {
	...
	sprintf( PrintData, "%d", player.level );
	TextOut( hdc, 176, 7, PrintData, strlen( PrintData ) );
	sprintf( PrintData, "%s", game_id );
	TextOut( hdc, 60, 7, PrintData, strlen( PrintData ) );
	sprintf( PrintData, "%d / %d", player.hp, player.MaxHp );
	TextOut( hdc, 47, 38, PrintData, strlen( PrintData ) );

	sprintf( PrintData, "%d", player.exp );
	TextOut( hdc, 185, 35, PrintData, strlen( PrintData ) );

	sprintf( PrintData, "%ds", player.skillTimer_1 );
	TextOut( hdc, 50, 97, PrintData, strlen( PrintData ) );

	sprintf( PrintData, "%ds", player.skillTimer_2 );
	TextOut( hdc, 90, 97, PrintData, strlen( PrintData ) );

	sprintf( PrintData, "%ds", player.skillTimer_3 );
	TextOut( hdc, 130, 97, PrintData, strlen( PrintData ) );

	sprintf( PrintData, "%ds", player.skillTimer_4 );
	TextOut( hdc, 170, 97, PrintData, strlen( PrintData ) );
}

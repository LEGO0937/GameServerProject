...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpszCmdLine, int nCmdShow ) {
	...
	hwnd = CreateWindow( "Window Class Name", "2015180014_TermProject", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 820, 840, NULL, NULL, hInstance, NULL );
	...
	case WM_PAINT:
		...
		for ( int i = 0; i < MAX_Chat; ++i ) {
			if ( chat_enabled[i] == true ) {
				...
				SetBkMode( mem0dc, OPAQUE );
				SetTextAlign( hdc, TA_TOP );
				wsprintf( isDebugData, "%ws",  npc[chat_ci[i]].message );
...
void init() {
	...
	if ( strcmp( buf, "False" ) == 0 ) {
		MessageBox( cpy_hwnd, "DB에서 ID를 확인할 수 없습니다.\n프로그램을 종료합니다.", "DB 오류", MB_OK );
		exit( -1 );
	}
